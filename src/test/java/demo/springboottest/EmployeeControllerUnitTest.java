...
public class EmployeeControllerUnitTest {
    ...
    public void whenPostEmployee_thenCreateEmployee() throws Exception {
        Employee employee1 = new Employee("Employee 1", 50000);
    ...
    public void givenEmployees_whenGetEmployees_thenReturnJsonArray() throws Exception {
        ...
        Employee employee2 = new Employee("Employee 2", 70000);
        Employee employee3 = new Employee("Employee 3", 80000);

        List<Employee> allEmployees = Arrays.asList(employee1, employee2, employee3);

        given(service.getAllEmployees()).willReturn(allEmployees);

        mvc.perform(get("/rmtest/employees").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(3))).andExpect(jsonPath("$[0].name", is(employee1.getName())))
